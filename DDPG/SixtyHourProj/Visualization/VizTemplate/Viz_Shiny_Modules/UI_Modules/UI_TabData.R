...
## functions exported from this module
    ...
    modules::export("DisplaySampleInvestorGoals")
    ...
    funcs_MVUITD_DC)
{
  ...
  theUI <- shiny::fluidRow(
    ...
    semantic.dashboard::box(
      ...
      semantic.dashboard::column(
        width = 10,
        rhandsontable::rHandsontableOutput(outputId = "outputID_TabData_SampleInputInvestorGoals")))
...
DisplaySampleInvestorGoals <- function(
  ## the first three arguments of the function have to be input, output and session
  input, output, session,
  ## the regular function arguments
  data_MVUITD_DSIG,
  funcs_MVUITD_DSIG,
  reactiveInputs_MVUITD_DSIG)
{
  nameFunctionForMessage <- "Mod_Viz_Server_TabData$DisplaySampleInvestorGoals() "
  reportr::setOutputLevel(reportr::OL$Error)
  
  theGoals <- shiny::isolate(expr = reactiveInputs_MVUITD_DSIG[["InvestorGoals"]])
  theSampleData <- theGoals
  theOutput <- rhandsontable::rhandsontable(
    data = theSampleData, 
    width = 800) 
  #      height = 300) 

    ...
    reactiveInputs_MVSRV_OE_RUI)
{
  ...
  theObjReactive_UserInputs <- ObserveEvents_GoalSeries(
    input, output, session,  
    inputs_MVSRV_OE_GS = inputs_MVSRV_OE_RUI,
    reactiveInputs_MVSRV_OE_GS = theReactiveUserInputs)
  
  theObjReactive_UserInputs <- ObserveEvents_GoalSeriesOne(
    input, output, session,  
    inputs_MVSRV_OE_GSO = inputs_MVSRV_OE_RUI,
    reactiveInputs_MVSRV_OE_GSO = theReactiveUserInputs)
  
  theObjReactive_UserInputs <- ObserveEvents_GoalSeriesTwo(
    input, output, session,  
    inputs_MVSRV_OE_GST = inputs_MVSRV_OE_RUI,
    reactiveInputs_MVSRV_OE_GST = theReactiveUserInputs)
...
ObserveEvents_GoalSeries <- function(
  ## the first three arguments of the function have to be input, output and session
  input, output, session,  
  ## the regular function arguments
  inputs_MVSRV_OE_GS,
  reactiveInputs_MVSRV_OE_GS)
{
  nameFunctionForMessage <- "Mod_Viz_Svr_ReactiveVars$ObserveEvents_GoalSeries() "    
  reportr::setOutputLevel(reportr::OL$Error)
  
  theReactiveUserInputs <- reactiveInputs_MVSRV_OE_GS
  shiny::observeEvent(
    
    eventExpr = inputs_MVSRV_OE_GS$inputID_TabAnalysisOne_ThirdSubTab_SelectGoalSeries, 
    
    handlerExpr = {
      theReactiveUserInputs[["NameGoalSeries_TabAnalysisOne_ThirdSubTab"]] <- inputs_MVSRV_OE_GS$inputID_TabAnalysisOne_ThirdSubTab_SelectGoalSeries
    }) 
...
ObserveEvents_GoalSeriesOne <- function(
  ## the first three arguments of the function have to be input, output and session
  input, output, session,  
  ## the regular function arguments
  inputs_MVSRV_OE_GSO,
  reactiveInputs_MVSRV_OE_GSO)
{
  nameFunctionForMessage <- "Mod_Viz_Svr_ReactiveVars$ObserveEvents_GoalSeriesOne() "    
  reportr::setOutputLevel(reportr::OL$Error)
  
  theReactiveUserInputs <- reactiveInputs_MVSRV_OE_GSO
  shiny::observeEvent(
    
    eventExpr = inputs_MVSRV_OE_GSO$inputID_TabAnalysisOne_FourthSubTab_SelectGoalSeriesOne, 
    
    handlerExpr = {
      theReactiveUserInputs[["NameGoalSeriesOne_TabAnalysisOne_FourthSubTab"]] <- inputs_MVSRV_OE_GSO$inputID_TabAnalysisOne_FourthSubTab_SelectGoalSeriesOne
    }) 
  
  return(theReactiveUserInputs)
}

ObserveEvents_GoalSeriesTwo <- function(
  ## the first three arguments of the function have to be input, output and session
  input, output, session,  
  ## the regular function arguments
  inputs_MVSRV_OE_GST,
  reactiveInputs_MVSRV_OE_GST)
{
  nameFunctionForMessage <- "Mod_Viz_Svr_ReactiveVars$ObserveEvents_GoalSeriesTwo() "    
  reportr::setOutputLevel(reportr::OL$Error)
  
  theReactiveUserInputs <- reactiveInputs_MVSRV_OE_GST
  shiny::observeEvent(
    
    eventExpr = inputs_MVSRV_OE_GST$inputID_TabAnalysisOne_FourthSubTab_SelectGoalSeriesTwo, 
    
    handlerExpr = {
      theReactiveUserInputs[["NameGoalSeriesTwo_TabAnalysisOne_FourthSubTab"]] <- inputs_MVSRV_OE_GST$inputID_TabAnalysisOne_FourthSubTab_SelectGoalSeriesTwo
    }) 

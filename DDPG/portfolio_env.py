...
class PortfolioEnv(gym.Env):
    """
    An environment for financial portfolio optimization.
    Reference: (1) https://stable-baselines.readthedocs.io/en/master/index.html
               (2) https://github.com/notadamking/RLTrader
               (3) https://github.com/AminHP/gym-anytrading
               (4) https://arxiv.org/abs/1706.10059
               (5) https://github.com/awslabs/amazon-sagemaker-examples/tree/e1b5245ad4afb3ad895984a7f66eba73b4eec339/reinforcement_learning
    """
    ...
    def __init__(self,
        ...
        Params:
            ...
            trading_cost - cost of trade as a fraction   e.g. 0.0015 corresponding to 0.15%
            time_cost - cost of holding as a fraction
            window_length - how many past observations["history"] to return
            start_idx - The number of days from START_DATE('2015-01-02') of the dataset
            sample_start_date - The start date sampling from history  e.g. '2015-07-28'
        """
        ...
        self.window_length = window_length  # get the data for last # days
        self.num_stocks = history.shape[0]  # number of assets in the portfolio
        self.start_idx = start_idx
        self.csv_file = CSV_DIR             # result output: 'output/portfolio-management.csv'
        ...
        # Current State (input variables): open,high,low,close;
        ...
        # Following Action: portfolio weights of next period [cash_bias,w1,w2...] where wi are [0, 1] for each asset
    ...
    def _step(self, action):
        ...
        - Where wi is a portfolio weight from 0 to 1. The first is cash_bias
        """
    ...
    def render(self, mode='human', close=False):
        ...
        self._render_weight()
        plt.suptitle(
            "max_drawdown={: 2.2%}".format(self.mdd) + "  " +
    ...
    def _render_price(self):
        # first subplot: portfolio vs. mean market performance        
        self.worth_ax.clear()
        self.worth_ax.plot(self.df_info["portfolio_value"],'r-', label='portfolio')
        self.worth_ax.plot(self.df_info["market_value"],'g-',label='market')
    ...
    def _render_weight(self):
        # second subplot: total weight of portfolio
        self.weight_ax.clear()
        self.weight_ax.plot(self.df_info.index, self.df_sum, label='total weight')
        self.weight_ax.plot(self.df_info['weights1'], 'r-', label='cash', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights2'], 'g-', label='Mid-Cap', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights3'], 'b-', label='Top 200', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights4'], 'y-', label='S&P 500', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights5'], 'm-', label='Emerging Markets', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights6'], 'c-', label='Municipal Bond', alpha=0.5)
        self.weight_ax.plot(self.df_info['weights7'], 'k-', label='High Yield Bond', alpha=0.5)
        self.weight_ax.set_title("Weight")
        self.weight_ax.legend()
        plt.ylim(-0.05, 1.1)
...
class DataGenerator(object):
    ...
    def __init__(self, history, abbreviation, steps, window_length=50, start_idx=0, start_date=None):
        ...
        Args:
            ...
            steps: the total number of steps to simulate, default is 1 year (252 trading days)
            window_length: observation window, must be less than 50
            start_date: the date to start   e.g. '2015-07-28'
        """
...
class PortfolioSim(object):
    ...
    Reference: https://arxiv.org/abs/2002.10990
    """

    def __init__(self, steps, asset_names=list(), trading_cost=0.0015, time_cost=0.0):
    ...
    def _step(self, w1, y1, st):
        ...
        y1 - price relative vector also called return - e.g. [1.0, 0.9, 1.1]
        st - current step
        """
        ...
        dw1 = (y1 * w0) / (np.dot(y1, w0) + EPS)                # weights vector
        s1 = self.trading_cost * (np.abs(dw1 - w1)).sum()       # transaction cost for every change

        total_wealth = 1 + ADD_INVEST[0:st-1].sum()
        assert s1 < total_wealth, 'Cost is larger than current holding'

        p1 = (p0 + ADD_INVEST[st-1]) * (1 - s1) * np.dot(y1, w1)   # portfolio value
        p1 = p1 * (1 - self.time_cost)                             # add a cost to holding
        ...
        benchmark = np.fv(rate=rr, nper=st-1, pmt=-ADD_INVEST[0], pv=-1)
        P = (1-RHO)*benchmark + RHO*GAMMA*p0    
        reward = -LAMBDA*(P - p1)*(st / self.steps) - LAMBDA*ADD_INVEST[st-1]    # modified by our write-up

        # remember for next step
        self.w0 = w1
        self.p0 = p1

        # capital-preservation client for retirement goal
